...
import edu.umkc.solr.logging.LogWatcherConfig;
import edu.umkc.solr.update.UpdateShardHandlerConfig;
import edu.umkc.solr.util.DOMUtil;
import edu.umkc.solr.util.PropertiesUtil;
...
public class SolrXmlConfigImp implements ISolrXmlConfigImp
{
	private SolrXmlConfigArch _arch;

  public SolrXmlConfigImp (){
  }
  ...
  public NodeConfig fromSolrHome(Path solrHome) {
    return SolrXmlConfig.fromSolrHome(solrHome);
  }
  
  public static class SolrXmlConfig {
    ...
    public static NodeConfig fromFile(ISolrResourceLoader loader, File configFile) {
    ...
    public static NodeConfig fromString(ISolrResourceLoader loader, String xml) {
      return fromInputStream(loader, new ByteArrayInputStream(xml.getBytes(StandardCharsets.UTF_8)));
    }

    public static NodeConfig fromInputStream(ISolrResourceLoader loader, InputStream is) {
      try {
    ...
    public static NodeConfig fromSolrHome(ISolrResourceLoader loader, String solrHome) {
      return fromFile(loader, new File(solrHome, SOLR_XML_FILE));
    }

    public static NodeConfig fromSolrHome(Path solrHome) {
      ISolrResourceLoader loader = new SolrResourceLoaderImp(solrHome.toString());
