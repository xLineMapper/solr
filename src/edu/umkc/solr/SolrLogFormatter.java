...
public class SolrLogFormatter extends Formatter {
  ...
  public SolrLogFormatter() {
    ...
    methodAlias.put(new Method("edu.umkc.solr.update.processor.LogUpdateProcessor","finish"), "UPDATE");
    methodAlias.put(new Method("edu.umkc.solr.core.SolrCore","execute"), "REQ");
  }
  ...
  public void setShorterFormat() {
    ...
    methodAlias.put(new Method("edu.umkc.solr.update.processor.LogUpdateProcessor","finish"), "");
  ...
  Map<ISolrCore, CoreInfo> coreInfoMap = new WeakHashMap<>();    // TODO: use something that survives across a core reload?
...
***/
    ...
    ISolrCore core = req == null ? null : req.getCore();
...
//     sb.append("\nL").append(record.getSequenceNumber());     // log number is useful for sequencing when looking at multiple parts of a log file, but ms since start should be fine.
  ...
  private Map<String,Object> getReplicaProps(ZkController zkController, ISolrCore core) {
