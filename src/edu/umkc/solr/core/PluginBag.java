...
import edu.umkc.solr.util.CryptoKeys;
...
public class PluginBag<T> implements AutoCloseable {
   ...
   */
  public PluginBag(Class<T> klass, ISolrCore core, boolean needThreadSafety) {
    this.core = core;
    this.klass = klass;
   ...
   */
  public PluginBag(Class<T> klass, ISolrCore core) {
  ...
  PluginHolder<T> createPlugin(PluginInfo info) {
    if ("true".equals(String.valueOf(info.attributes.get("runtimeLib")))) {
      ...
      T inst = SolrCoreFactoryImp.SolrCore.createInstance(info.className, (Class<T>) meta.clazz, meta.getCleanTag(), null, core.getResourceLoader());
      initInstance(inst, info);
      return new PluginHolder<>(info, inst);
    }
  }

  public boolean alias(String src, String target) {
  ...
  public void init(Map<String, T> defaults, ISolrCore solrCore) {
    init(defaults, solrCore, solrCore.getSolrConfig().getPluginInfos(klass.getName()));
  }

  /**
   * Initializes the plugins after reading the meta data from {@link edu.umkc.solr.core.SolrConfig}.
   *
   * @param defaults These will be registered if not explicitly specified
   */
  public void init(Map<String, T> defaults, ISolrCore solrCore, List<PluginInfo> infos) {
    core = solrCore;
    for (PluginInfo info : infos) {
      PluginHolder<T> o = createPlugin(info);
      String name = info.name;
      if (meta.clazz.equals(SolrRequestHandler.class)) name = RequestHandlersFactoryImp.normalize(info.name);
  ...
  private static void registerMBean(Object inst, ISolrCore core, String pluginKey) {
    if (core == null) return;
  ...
  public static class LazyPluginHolder<T> extends PluginHolder<T> {
    ...
    private final ISolrCore core;
    protected ISolrResourceLoader resourceLoader;


    LazyPluginHolder(SolrConfig.SolrPluginInfo pluginMeta, PluginInfo pluginInfo, ISolrCore core, ResourceLoader loader) {
      super(pluginInfo);
      this.pluginMeta = pluginMeta;
      this.core = core;
      this.resourceLoader = (ISolrResourceLoader) loader;
    ...
    private synchronized boolean createInst() {
      ...
      T localInst = SolrCore.createInstance(pluginInfo.className, clazz, pluginMeta.getCleanTag(), null, resourceLoader);
      initInstance(localInst, pluginInfo);
      if (localInst instanceof SolrCoreAware) {
        SolrResourceLoaderImp.assertAwareCompatibility(SolrCoreAware.class, localInst);
        ((SolrCoreAware) localInst).inform(core);
      }
      if (localInst instanceof ResourceLoaderAware) {
        SolrResourceLoaderImp.assertAwareCompatibility(ResourceLoaderAware.class, localInst);
        try {
  ...
  public static class RuntimeLib implements PluginInfoInitialized, AutoCloseable {
    ...
    private final ICoreContainer coreContainer;
    ...
    public RuntimeLib(ISolrCore core) {
    ...
    public static List<RuntimeLib> getLibObjects(ISolrCore core, List<PluginInfo> libs) {
