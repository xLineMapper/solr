...
import edu.umkc.solr.cloud.CloudConfigSetService;
...
public abstract class ConfigSetService {
  ...
  public static ConfigSetService createConfigSetService(NodeConfig nodeConfig, ISolrResourceLoader loader, ZkController zkController) {
  ...
  protected final ISolrResourceLoader parentLoader;

  /**
   * Create a new ConfigSetService
   * @param loader the CoreContainer's resource loader
   */
  public ConfigSetService(ISolrResourceLoader loader) {
  ...
  public final ConfigSet getConfig(CoreDescriptor dcore) {
    ...
    ISolrResourceLoader coreLoader = createCoreResourceLoader(dcore);
   ...
   */
  protected SolrConfig createSolrConfig(CoreDescriptor cd, ISolrResourceLoader loader) {
  ...
  /**
   * Create a ISolrResourceLoader for a core
   * @param cd the core's CoreDescriptor
   * @return a ISolrResourceLoader
   */
  protected abstract ISolrResourceLoader createCoreResourceLoader(CoreDescriptor cd);
  ...
  public static class Default extends ConfigSetService {
     ...
     */
    public Default(ISolrResourceLoader loader, String configSetBase) {
      super(loader);
      this.configSetBase = resolveBaseDirectory(loader, configSetBase);
    }

    private File resolveBaseDirectory(ISolrResourceLoader loader, String configSetBase) {
    ...
    @Override
    public ISolrResourceLoader createCoreResourceLoader(CoreDescriptor cd) {
      String instanceDir = locateInstanceDir(cd);
      return new SolrResourceLoaderImp(instanceDir, parentLoader.getClassLoader(), cd.getSubstitutableProperties());
    }
  ...
  public static class SchemaCaching extends Default {
    ...
    public SchemaCaching(ISolrResourceLoader loader, String configSetBase) {
