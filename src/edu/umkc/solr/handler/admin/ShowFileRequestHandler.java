...
import comp.RawResponseWriter.RawResponseWriterImp;
 ...
 * <br>
 * <pre>
 * &lt;requestHandler name="/admin/file" class="edu.umkc.solr.handler.admin.ShowFileRequestHandler" &gt;
 *   &lt;lst name="defaults"&gt;
 ...
 *
 * <p>
 * The ShowFileRequestHandler uses the {@link RawResponseWriterImp} (wt=raw) to return
...
public class ShowFileRequestHandler extends RequestHandlerBase
{
  ...
  private void showFromZooKeeper(SolrQueryRequest req, SolrQueryResponse rsp,
      ICoreContainer coreContainer) throws KeeperException,
    ...
    if (children.size() > 0) {
      ...
      rsp.add(RawResponseWriterImp.CONTENT, content);
    }
  ...
  public static File getAdminFileFromFileSystem(SolrQueryRequest req, SolrQueryResponse rsp,
                                                Set<String> hiddenFiles) {
    File adminFile = null;
    final ISolrResourceLoader loader = req.getCore().getResourceLoader();
